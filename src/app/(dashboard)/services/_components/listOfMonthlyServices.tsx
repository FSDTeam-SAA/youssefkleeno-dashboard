...
const ListOfMonthlyServices = () => {
  ...
  // ✅ Fetch Monthly Subscription Services
  const { data, isLoading, error } = useQuery<ServiceResponse>({
    queryKey: ["monthlyServices", currentPage],
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/service?washType=Monthly Subscription&page=${currentPage}&limit=${itemsPerPage}`
      );
      if (!res.ok) throw new Error("Failed to fetch monthly services");
      return res.json();
    },
  });
  ...
  if (error)
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load monthly services
      </p>
    );
            ...
            <TableRow key={service._id} className="hover:bg-muted/20">
              ...
              <TableCell>
                <div className="flex justify-center items-center gap-3">
                  <Link href={`/services/edit-monthly-washService/${service._id}`}>
...
export default ListOfMonthlyServices;
